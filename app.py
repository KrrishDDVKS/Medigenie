...
def load_lottie_url(path: str):
    try:
        ...
        st.error(f"Error: The file at path {path} was not found.")
    except json.JSONDecodeError:
        st.error(f"Error: The file at path {path} is not a valid JSON file.")
...
st.markdown(
    "<h1 style='font-family: \"Special Elite\"; font-size: 36px; color: #f4d35e; text-align: center;'>Medi Genie: A Multimodal Health Assistant</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<h2 style='font-family: \"Special Elite\"; font-size: 24px; color: #e0d8c3; text-align: center;'>A medical mystery-solving assistant for your skin</h2>",
    unsafe_allow_html=True
)

# --- Chatbot Section ---
st.markdown(
    "<h3 style='font-family: \"Special Elite\"; font-size: 20px; color: #ffffff; text-align: left;'>💬 Interrogate Skin Scout</h3>",
    unsafe_allow_html=True
)

if prompt := st.chat_input():
    st.markdown('<div class="typing">🕵️‍♂️ Skin Scout is investigating your case...</div>', unsafe_allow_html=True)
    st.markdown("📜 Case Notes: I'm still in training. Full AI diagnosis is under construction. 🔧")
